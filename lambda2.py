...
def lambda_handler(event, context):
    """
    Lambda entry point for Prometheus Alertmanager webhook.
    Event is expected to have 'alerts'.
    """
    print(event)
    body = event.get("body", "{}") 
    try:
    ...
    if not alerts:
        return {"statusCode": 200, "body": "No alerts received"}

    # Run agent synchronously (Lambda threads have limitations)
    try:
        monitor_agent = Agent(
            name="MonitorAgent",
        ...
        result = monitor_agent(str(alerts))
        if hasattr(result, "message") and "content" in result.message:
            output_text = result.message["content"][0].get("text", "")
        else:
            output_text = str(result)
        # Optionally log result to CloudWatch
        print("Agent Response:", output_text)
        return {
            "statusCode": 200,
            "body": json.dumps({"status": "agent executed", "result": output_text})
        }
    except Exception as e:
